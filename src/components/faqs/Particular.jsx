...
console.log(data.question)
  return (
    ...
    <div onClick={()=>{
      console.log(active)
      setActive(!active)
     }} className={`toggle my-2  overflow-hidden   rounded-[35px] transition-all duration-300 ease-in-out border ${active ? 'min-h-[120px]  ' : 'h-[80px] '}`}>
    {/* Your content here */}
  
  
  
  
      <div className={` ${active ? "bg-[#F76900]" : "bg-white"} transition-all duration-300 ease-in-out question  h-[80px] w-[100%]  rounded-[38px]  flex  justify-between  items-center p-3`}>
